...
if menu == "Agenda":
    ...
    with st.spinner("Buscando locações..."):
        conn = conectar()
        try:
            cursor = conn.cursor(dictionary=True)
            query_loc = """
                        ...
                        if res:
                            with st.form(key=f"form_ed_reserva_{id_edit}"):
                                ...
                                if btn_salvar:
                                    ...
                                    sql_update = "UPDATE alugueis SET data_inicio=%s, valor_final=%s, valor_pago=%s, observacoes=%s WHERE id = %s"
    ...
    if st.session_state['usuario_nome'] == "pedro":
        with st.spinner("Calculando faturamento..."):
            conn = conectar()
            try:
